...
class jobapp(tk.Tk): #This class holds all the code and allows the code to be split into parts
    def __init__(self, *arg, **kwarg):
        ...
        reference_number_label = ttk.Label(self.link_ref, text="Reference Number :", width = 15)
        ...
        cancel_button= ttk.Button(self.link_ref, text = "Cancel", width = 20, command = lambda : self.next_page(self.link_ref, self.student))
        ...
        setup_e_label.grid(row = 3, column = 4)
        ...
        setup_r_label.grid(row = 3, column = 5)
    ...
    def finish_t(self, job_t, email, phone): #Adds information when a new reference is added
        if "@" not in email:
            invalid_email_r_label = ttk.Label(self.reference_setup, text="Please enter a valid email", width = 20)
            ...
            if len(phone) < 9:
                ...
                with open("CurrentLogged.txt") as h:
                    ...
                    with open(first[:-1]+"2.txt") as j:
                        ...
                        placeholder28 = ttk.Label(self.reference, text = "                  ", width= 8)
                        placeholder28.grid(row = 2, column = 3, pady = 5)
                    ...
                    with open("CurrentLogged.txt") as k:
                            ...
                            with open(first[:-2]+"2.txt") as j:
                                ...
                                if "RefNo" in z.read():
                                    ...
                                    reference_number_r_label = ttk.Label(self.reference, anchor = "center", text=("Reference Number: "+ topline.strip("RefNo")), font = ("Helvetica 10 underline"))
                                    reference_number_r_label.grid(row = 1, column = 6)
                                placeholder29 = ttk.Label(self.reference, text = "                  ", width= 8)
                                placeholder29.grid(row = 100, column = 4, pady = 5)
    ...
    def accept(self, student): #Adds the accepted student to an external list for the company
        with open("Logged.txt") as h:
            lines = h.read()
            first = lines.split('\n', 0)[0].strip()
            if student in open(first[:-1]+".txt").read(): #Checks if the user entered is actually applied
                try:
                ...
                import acceptedapplicants #Imports another code which shows the employer their accepted applicants
            else: 
                false_applicant_e_label = ttk.Label(self.employer, text = "Please enter a real applicant", width = 30)
                false_applicant_e_label.grid(row = 9, column = 1, columnspan= 2)
    ...
    def search_application(self, user, refno): #Checks if the user has applied for a company
        if len(user) == 0:
            no_user_lr_label = Label(self.link_ref, text="Please Enter a User", width = 20)
            no_user_lr_label.grid(row = 24, column = 3, columnspan=2)
        else:
