...
def login():
    ...
    try:
        ...
        pass
    open_u = open("Usernames.txt")
    open_p = open("Passwords.txt")
    if u_p in open_u.read() and act_type == 1:
        try:
            f = open("Logged.txt", "x")
        except FileExistsError:
            pass
        write_logs = open("Logged.txt", "w")
        write_logs.write(username+password+"1"+"\n")
        write_logs.close
        student_page()
    if u_p in open_u.read() and act_type == 2:
        try:
            f = open("Logged.txt", "x")
        except FileExistsError:
            pass
        write_logs = open("Logged.txt", "w")
        write_logs.write(username+password+"2"+"\n")
        write_logs.close
        reference_page()
    if u_p in open_u.read() and act_type == 3:
        try:
            f = open("Logged.txt", "x")
        except FileExistsError:
            pass
        write_logs = open("Logged.txt", "w")
        write_logs.write(username+password+"3"+"\n")
        write_logs.close
        employer_page()
    else:
        ...
        if username in open("Usernames.txt","r"):
            ...
            if var.get() == 1:
                ...
                try:
                    ...
                    pass
                reference_write = open(username+".txt", "a")
                reference_write.write("ReferenceNo"+reference_no)
                reference_write.close()
                try:
                    f = open("References.txt", "x")
                except FileExistsError:
                    pass
                write_r = open("References.txt", "w")
