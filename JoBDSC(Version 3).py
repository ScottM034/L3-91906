...
class jobapp(tk.Tk): #This class holds all the code and allows the code to be split into parts
    def __init__(self, *arg, **kwarg):
        ...
        con = tk.IntVar()
        lbl1 = ttk.Label(self.login, text="Login", width = 15, font = ("Arial")) #Adding a bit of text in a specific page
        ...
        lbl6 = ttk.Label(self.login, text = "Account Type :", font = ("Arial"))
        ...
        lbl4 = ttk.Label(self.login, text="Sign-Up", width = 15, font = ("Arial"))
        ...
        comment_title.grid(row = 2, column = 2, columnspan= 2)
        lbl21 = ttk.Label(self.ref_comment, text = " ")
        ...
        lbl1 = ttk.Label(self.link_ref, text="Link a Reference", width = 20, font = ("Arial"))
        lbl1.grid(row = 3, column = 3, columnspan=2)
        lbl2 = ttk.Label(self.link_ref, text="Name :", width = 15, font = ("Arial"))
        lbl2.grid(row = 8, column = 2)
        ...
        lbl21.grid(row = 9, column = 1)
        lbl3 = ttk.Label(self.link_ref, text="Reference Number :", width = 15, font = ("Arial"))
        lbl3.grid(row = 10, column = 2)
        ...
        link1.grid(row = 0, column = 3, columnspan= 2)
        ...
        btn2.grid(row = 0, column = 6, columnspan=2)
        ...
        try:
            ...
            pass
        title_label = ttk.Label(self.student, text = "Apply To Job", width = 20)
        ...
        lbl24 = ttk.Label(self.employer_setup, text="Put your reference number on the website!", width=50)
        lbl24.grid(row = 31, column =3, columnspan= 4)
        lbl24 = ttk.Label(self.employer_setup, text="Logout after signup to then see it update with anyone who has applied", width=70)
        lbl24.grid(row = 32, column =3, columnspan= 4)


        #Employer Page
        logout = ttk.Button(self.employer, text="Logout", command = lambda : self.next_page(self.employer, self.login), width=14)
        logout.grid(row = 0, column = 6, columnspan=2)
        try:
        ...
        view_list = ttk.Button(self.employer, text = "View Applicants", command = lambda : self.view_applicants(), width = 20)
        view_list.grid(row = 3, column = 3, columnspan= 3)
        applicant_name = ttk.Label(self.employer, text = "Applicant Name: ")
        applicant_name.grid(row = 5, column = 2)
        applicant_name_entry = ttk.Entry(self.employer, width = 30)
        applicant_name_entry.grid(row = 5, column = 3)
        accept_applicant = ttk.Button(self.employer, text = "Accept", width = 10, command = lambda : self.accept(applicant_name_entry.get()))
        accept_applicant.grid (row = 8, column = 3, columnspan= 2)
        accepted_button = ttk.Button(self.employer, text = "Accepted Applicants", width = 20, command = lambda : self.accepted_list())
        accepted_button.grid(row = 13, column = 3)
    ...
    def finish(self, job_t, location, email, phone): #From the user's intital sign-up we create a file that has all his information
        if "@" not in str(email): #Checking if the email the user entered is invalid
            ...
            if len(phone) < 9: #Checking if the mobile number the user entered is invalid
                lbl30 = ttk.Label(self.employer_setup, text="Please enter a valid Mobile Number", width = 25)
                ...
                with open("CurrentLogged.txt") as h:
                    ...
                    with open(first[:-1]+"3.txt") as j:
                        top = j.read()
                        topline = top.split('\n', 0)[0]
                        lbl22 = ttk.Label(self.employer, text=("Your reference number is "+ topline.strip("RefNo")))
                        lbl22.grid(row = 1, column = 4, columnspan= 3)
                        new_space = ttk.Label(self.employer, text = "                  ", width= 8)
                        new_space.grid(row = 2, column = 4, pady = 5)
    ...
    def finish_s(self, job_t, location, email, phone):
        if "@" not in email:
            ...
            if len(phone) < 9:
                lbl30 = ttk.Label(self.student_setup, text="Please enter a valid Mobile Number", width = 25)
                lbl30.grid(row = 28, column = 3, columnspan=2)
            else:
                with open("CurrentLogged.txt") as h:
                    lines = h.read()
    ...
    def finish_t(self, job_t, email, phone):
        if "@" not in email:
            ...
            if len(phone) < 9:
                lbl30 = ttk.Label(self.reference_setup, text="Please enter a valid Mobile Number", width = 25)
                lbl30.grid(row = 28, column = 3, columnspan=2)
            else:
                with open("CurrentLogged.txt") as h:
                    lines = h.read()
                    ...
                    with open(first[:-1]+"2.txt") as j:
                        top = j.read()
                        topline = top.split('\n', 0)[0]
                        lbl22 = ttk.Label(self.reference, text=("Your reference number is "+ topline.strip("RefNo")))
                        lbl22.grid(row = 0, column = 2, columnspan= 3)
                        new_space = ttk.Label(self.employer, text = "                  ", width= 8)
                        new_space.grid(row = 2, column = 4, pady = 5)
    ...
    def search(self, user, refno): #This function checks if the user entered a valid reference 
        if len(user) == 0:
            lbl10 = Label(self.student, text="Please Enter a User", width = 20)
            ...
            if user in open_u.read():
                ...
                if ("RefNo"+str(refno)) in open_e.read():
                    try:
                        ...
                        pass
                    write_reference = open(user+".txt", "a")
                    write_reference.write(first+"\n")
                    write_reference.close()
                else:
                    lbl10 = Label(self.student, text="That User does not exist", width = 42)
    ...
    def user_check_s(self, user, password, act_type):
        if len(password) < 8:
            ...
            if user+str(act_type) in username_check.read() and act_type != 3:
                lbl10 = ttk.Label(self.login, text="Username already exists, please try again", width = 30)
                ...
                if act_type == 1:
                    ...
                    for i in range(line_count):
                        ...
                        if first[:-1] in referencelook.read():
                            new_label = ttk.Label(self.reference, text = stud, width=15, font = ("Arial"))
    ...
    def accept(self, student):
        with open("Logged.txt") as h:
            ...
            if student in open(first[:-1]+".txt").read():
                ...
                false_applicant.grid(row = 9, column = 2, columnspan= 4)
                time.sleep(3)
                ...
                if (refno) in open_e.read():
                    if first in open(user+".txt","r").read():
                        ...
                        try:
                            ...
                            pass
                        write_applicant = open(user+".txt", "w")
                        write_applicant.write(first+"\n")
                        write_applicant.close()
                        ...
                        contact_label = ttk.Label(self.student, text = "You will be contacted later", width = 35)
