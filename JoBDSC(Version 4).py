...
class jobapp(tk.Tk): #This class holds all the code and allows the code to be split into parts
    def __init__(self, *arg, **kwarg):
        ...
        view_list = ttk.Button(self.employer, text = "View Applicants", command = lambda : self.view_applicants(), width = 30)
        view_list.grid(row = 3, column = 3)
        placeholder = ttk.Label(self.employer, text = ' ')
        ...
        accept_applicant = ttk.Button(self.employer, text = "Accept", width = 30, command = lambda : self.accept(applicant_name_entry.get()))
        accept_applicant.grid (row = 8, column = 3)
        placeholder = ttk.Label(self.employer, text = ' ')
        placeholder.grid(row = 9 ,column=0)
        accepted_button = ttk.Button(self.employer, text = "Accepted Applicants", width = 30, command = lambda : self.accepted_list())
            ...
            if len(phone) < 9: #Checking if the mobile number the user entered is invalid
                ...
                with open("CurrentLogged.txt") as h:
                    ...
                    with open(first[:-1]+"3.txt") as j:
                        top = j.read()
                        topline = top.split('\n', 0)[0]
                        lbl22 = ttk.Label(self.employer, text=("Your reference number is "+ topline.strip("RefNo")), anchor='S')
