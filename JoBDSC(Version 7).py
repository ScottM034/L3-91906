            ...
            if len(phone) < 9: #Checking if the mobile number the user entered is invalid
                ...
                with open("CurrentLogged.txt") as h:
                    ...
                    with open(first[:-1]+"3.txt") as j:
                        top = j.read()
                        topline = top.split('\n', 0)[0]
                        lbl22 = ttk.Label(self.employer, anchor = "center", text=("Referece Number: "+ topline.strip("RefNo")))
                ...
                if act_type == 1:
                    ...
                    with open("Logged.txt") as h:
                        ...
                        with open(first[:-1]+"3.txt") as j:
                            top = j.read()
                            topline = top.split('\n', 0)[0]
                            lbl22 = ttk.Label(self.employer, anchor = "center", text=("Reference Number: "+ topline.strip("RefNo")), font = ("Helvetica 10 underline"))
                            lbl22.grid(row = 1, column = 6)
                            new_space = ttk.Label(self.employer, text = "                  ", width= 8)
                            new_space.grid(row = 2, column = 4, pady = 5)
                        try:
